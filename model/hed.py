...
def treat(x):
        return x - torch.tensor(data=[0.4, 0.45, 0.478], dtype=x.dtype, device=x.device).view(1, 3, 1, 1)
...
class HED(nn.Module):
    ...
    def forward(self, x : torch.Tensor, return_side_outputs = True) -> torch.Tensor:
        if len(x.shape) == 3:
            x = torch.unsqueeze(x, 0)
        x = x * 255
        x = x - torch.tensor(data=[104.00698793, 116.66876762, 122.67891434], dtype=x.dtype, device=x.device).view(1, 3, 1, 1)
        ...
        if not(return_side_outputs):
            ...
            return final_response, stacked # type: ignore '# error : ignore'
    ...
    def load_pre_treined_weigths_vgg(self, device):
        ...
        vgg_layers = vgg16_pretrained.features # type: ignore
